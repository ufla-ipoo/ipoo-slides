...
Veja que usando um `ArrayList` a classe `OrganizadorDeMusica` fica muito simples.
...
Outra coisa importante é que o fato do `ArrayList` guardar o número de objetos que ele guarda, traz consequências importantes.
...
- Podemos ter: *ArrayList de Strings*, *ArrayList de Pessoas*, *ArrayList de Retângulos*, enfim, de qualquer classe que precisarmos.
  - Cada um deles é um tipo diferente.
- Por enquanto, vamos usar apenas a classe `ArrayList`, mas existem diversas outras coleções também implementadas como classes genéricas.
...
- Não! Justamente porque as variáveis têm tipos diferentes.
...
- Mas que pudéssemos tocar realmente as músicas.
...
- Os dois primeiros métodos recebem o nome do arquivo de música por parâmetro.
  ...
  - Já o segundo começa a tocar a música em background e já retorna para o organizador.
...
Se quiser, você pode usar as músicas do repositório [organizador-musicas-v2](https://github.com/ufla-ipoo/amostras-musicas).
...
Nós sabemos como usar o básico de uma coleção em Java.
...
- Precisamos saber apenas os nomes, tipos de parâmetro e de retorno de alguns métodos importantes.
...
::: {.callout-note title="Exercício" icon=false}
Cria um `ArrayList` de Strings no bloco de códigos do BlueJ, digitando as duas linhas abaixo.
...
- Mas nós não precisamos fazer a mesma coisa com todos os objetos, necessariamente.
