...
- Você estudou em casa **todo o conteúdo** de todas as aulas **teóricas** anteriores?
...
Veremos o princípio da **abstração**, na prática, ao utilizarmos uma classe da [Biblioteca de Classes]{.alert} do Java.
...
- Não é a nossa classe que vai guardar as músicas.
...
## Notação diamante {auto-animate=true}
...
Veja que nós usamos a classe `ArrayList` **sem nos preocuparmos** exatamente sobre **como a classe funciona**.
...
Ao usar o projeto `organizador-musicas-v1` você notou que, para **listar e remover músicas**,

- foi necessário passar **parâmetros começando em zero**.
...
É importante lembrar que, ao **remover um objeto** de um `ArrayList`, 

- os **objetos** que estavam **depois dele mudam de posição**.
...
Se tivermos uma **classe** implementada **para tocar arquivos de aúdio**,

- nossa classe organizadora não precisa saber nada sobre como fazer isso.
...
É o que chamamos de **bibliotecas de classes de terceiros** ([third-party libraries]{.alert}).
...
Em uma nova versão do projeto, usaremos classes da biblioteca `javazoom.net` 

- para escrever a nossa própria classe tocadora de música.
...
A ideia agora é ter um método na classe `OrganizadorDeMusicas` 

- que exiba no terminal todos os nomes de arquivos guardados na coleção.
...
## O loop for-each {auto-animate=true}
...
Faremos essa melhoria mais adiante neste capítulo!

# Buscando em uma coleção {background-color="#40666e"}

##

Nós vimos que o loop **for-each** é usado para **processar uma coleção inteira**.
...
A **sintaxe** do [`while`]{.alert} em Java é **igual à do C++** que vocês viram na disciplina IALG.

. . .

Como exemplo, os trechos de código do próximo slides fazem a mesma coisa, 

- mas um usando **for-each** 
- e o outro `while`.
...
Na próxima aula teórica continuaremos evoluindo nosso projeto.

- Especialmente através da criação de uma classe para representar músicas.

# Sobre a Aula 4.2 (em casa) {background-color="#40666e"}

##

Na verdade a Aula 4.2 não é bem uma aula. 

- O objetivo é reforçar a necessidade de estudarem em casa entre as aulas 4.1 e 4.3 para reforçar o entendimento do Capítulo 3. 

. . .

Além de rever o material da primeira aula, é recomendável fazer o exercício do **projeto `canal`**,

- que será o primeiro da Aula Prática 4.4.

##

O que foi visto na Aula 4.1 é suficiente para fazer os três primeiros exercícios. 

- Mas, caso se sinta confiante, tente fazer também dos demais exercícios.
- Os enunciados dos exercícios estão no arquivo README.md do projeto.
  - Dica: é mais fácil visualizá-los no próprio GitHub.
