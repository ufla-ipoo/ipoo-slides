...
- Quem quer vender itens, insere-os no leilão através do método `cadastrarLote`, 
...
A classe ilustra o uso de `ArrayList` e loop usando _for-each_.
...
Veremos agora uma característica comum em programas OO:
...
- a versao original tem a vantagem de evitar a declaração de uma variável de uso limitado.
  - Nessa versão, chamamos o objeto de **anônimo** (sem nome) pois ele é passado diretamente para o método.
...
Como as variáveis `lance`, `ofertante` e `nome` estão sendo usadas simplesmente como estágios intermediários 
para chegar ao nome do ofertante,

- é comum ver sequências como esta compactadas por meio do uso de referências a objetos anônimos. 

. . .

Por exemplo, podemos obter o mesmo efeito com o seguinte código:
