...
- Aprendemos isso usando [leitura de código]{.alert}.
...
Já a classe `Email` é bem simples.
...
Precisamos usar `this` aqui por causa da [sobrecarga de nomes]{.alert}.
...
- Logo, o parâmetro foi declarado "mais perto" da linha de código.
  - Portanto, na linha de código destacada acima, estamos referenciando o **parâmetro `remetente`**.
...
- **atributo** chamado `remetente` _recebe_ **parâmetro** chamado `remetente`.
...
## Definindo pontos de parada (*breakpoints*) {auto-animate=true}

. . .

Um [*breakpoint*]{.alert} é uma marcação ligada a uma linha de código na qual o depurador pausa a execução do programa.
...
![](imagens/breakpoint_bluej.png){.relative width="50%"}

## Definindo pontos de parada (*breakpoints*) {auto-animate=true}


![](imagens/breakpoint_bluej.png){.r-stretch}  

- Breakpoints só podem ser colocados em linhas de código executáveis.
  - Portanto, não é possível colocá-los em assinatura de métodos, por exemplo.
...
![](imagens/depurador_bluej.png){r-stretch}
...
:::: {.callout-tip title="Atividade em aula" icon=false}
Use o botão [Avançar]{.alert} **uma única vez** para executar uma linha do método `imprimirProximoEmail`.
::::

. . .

No editor de código, a linha destacada agora é a próxima linha.

- E, na janela do depurador, vemos que a variável local `email` foi criada.
- Você pode até clicar duas vezes na variável para inspecionar o objeto.
...
Chegando nela, use o botão [Entrar em]{.alert} em vez de **Avançar**.
