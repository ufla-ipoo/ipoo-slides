...
Um **depurador** é um programa que permite que programadores possam executar seus programas um passo de cado vez.
...
- Um objeto servidor de email devem ser criado primeiro para ser usado pelos clientes de e-mail.
...
- Não há [variáveis estáticas]{.alert}.
...
Não se esqueça de garantir que o assunto do e-mail será exibido ao exibir um e-mail. E modifique a classe `ClienteDeEmail` conforme necessário.
...
- Adicione um atributo `anexo`, do tipo String, para as mensagens de e-mail.
  - Vamos simular que existe um anexo guardando um nome de arquivo nesse atributo.
...
Nossos sistema de e-mails permite imprimir um e-mail por vez, mas seria interessante se pudéssemos ver todos os nossos e-mails de uma vez.

::: {.nonincremental}
Crie então um método `imprimirTodosEmails` na classe `ClienteDeEmail` que permita fazer isso.

- Você precisará usar a [palavra-chave `null`]{.alert} que é valor de uma variável que não referencia nenhum objeto.
- Use alguma forma de separação entre um e-mail do outro.
...
Caso contrário, apenas os e-mails do remetente recebido por parâmetro devem ser exibidos.
...
::: {.nonincremental}
A ideia é parecida com o que fizemos antes, para os remetentes, mas agora, ao chamar o método, não precisaremos passar o assunto completo. Além disso, vamos mostrar os e-mails ignorando maiúsculas/minúsculas

- Por ex.: se chamarmos o método passando "poo" como parâmetro, os dois primeiros e-mails abaixo seriam exibidos e o terceiro não.
  - Assunto do e-mail 1: "Notas de POO".
  - Assunto do e-mail 2: "Videoaulas de IPOO e outros avisos".
  - Assunto do e-mail 3: "Rebobina"
:::
...
::: {.nonincremental}
No design atual do `VisorDeRelogio`, um objeto `VisorDeRelogio` é responsável por detectar quando um objeto `VisorDeNumero`
...
Seria possível que um objeto `VisorDeNumero` informasse outro que ele voltou para zero e que o outro objeto `VisorDeNumero` deveria então
ser incrementado? 
Por exemplo, fazer com que o objeto minutos informasse o objeto hora que outra hora se passou ou fazer com que o objeto segundos informasse ao objeto minutos que outros sessenta segundos se passaram.

- O método `tiqueTaque` deveria interagir com quais objetos?
- O que o objeto hora deveria fazer quando um dia inteiro tiver passado?

Discuta os problemas envolvido neste design alternativo e, se você realmente sentir vontade de desafiar, tente implementá-lo!
:::
::::
...
::: {.callout-important title="Entrega no Campus Virtual"}
Acesse, no Campus Virtual, a atividade correspondente a essa parte dos exercícios e envie o código do projeto `visor-relogio-melhor` da forma que ficou após as suas alterações (compacte a pasta em um arquivo `.zip` para enviar);
:::
