...
- Os três atributos são do tipo `int`.
  ...
  - Como nossa máquina guarda os valores em centavos, o tipo de dados é apropriado.
...
::: {.callout-note title="Exercício" icon=false}
Pelo que você sabe das convenções de nomes de classes, nos dois exercícios anteriores quais tipos de atributos você acha que são classes?
:::
...
- E não podemos chamar o construtor para um objeto já criado.
...
Existe uma relação direta entre o que acontece no corpo^[Obs.: corpo de um construtor ou de um método é o bloco de código que contém sua implementação. Ou seja, é tudo que tem entre `{` e `}` logo após a assinatura.] de um construtor e os atributos da classe.
...
Mas porque o preço é recebido de fora?