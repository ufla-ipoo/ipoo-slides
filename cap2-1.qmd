...
- Não podemos, por exemplo, guardar o valor de uma expressão que resulta em um número inteiro em uma variável string.

- Seguindo a mesma ideia, não podemos passar um valor inteiro como argumento para um parâmetro string de um construtor, por exemplo.
...
- Note que **construtores não têm tipo de retorno** enquanto métodos têm (aparecem antes do nome do método).
  ```{.java code-line-numbers="false"}
  public MaquinaIngressos(int custoIngresso)
  public int obterPreco()
  ```
...
- Obs.: nesse material vou preferir usar sempre `obter`, mas fique para escolher o que prefere.
:::
:::
...
::: {.column width="80%"}
::: {.callout-note title="Convenção" appearance="simple" icon=false}
Em inglês, assim como os métodos de acesso começam com `get`, os métodos modificadores começam com `set`
