...
::: {.column width="70%"}
- Para construir os slides usei também a 6ª edição do livro.
  - Que está disponível apenas em inglês.
  - Portanto, os slides podem ter coisas a mais ou ligeiramente diferentes do livro.
...
::: {.callout-note title="Exercício" icon=false}
Tente entender bem o funcionamento da máquina interagindo com o objeto no BlueJ antes de ver o código na próxima seção.
...
- Uma pequena [parte externa]{.alert} que dá nome à classe (aparece com fundo verde no BlueJ).
- E uma [parte interna]{.alert} muito maior que faz todo o trabalho.

. . .

A parte externa tem o seguinte trecho de código
...
A **parte externa** de todas as classes se parecem muito, já que o objetivo é [nomear a classe]{.alert}.
...
- [Construtores]{.alert} são responsáveis por garantir que um objeto seja configurado corretamente quando ele é criado.
...
Você não é obrigado a seguir essa ordem, mas ela deixa seu **código mais claro**.
...
Na primeira coloque os nomes dos atributos, na segunda os nomes dos construtores e, na terceira, os métodos.
...
::: {.column width="20%"}
![](imagens/messi_fifa_ratings.png){width="40%"}
:::
...
::: {.column width="30%"}
![](imagens/casal_fazendo_careta.png)
:::

::: {.column width="35%"}

Quais seriam os atributos físicos de cada pessoa?
...
Atributos são pequenos espaços dentro de um objeto que podem ser usados para guardar dados de forma permanente.
...
## Quiz 2.1 {background-color="#f7fad1"}
...
```{.java code-line-numbers="false"}
  /**
   * Cria uma máquina que emite ingressos de um dado
   * preço. Note que o preço deve ser maior que zero,
   * e não tem nenhuma verificação para garantir isso.
   */
...
- *Obs.: corpo de um construtor ou de um método é o bloco de código que contém sua implementação.* 
  - *Ou seja, é tudo que tem entre `{` e `}` logo após a assinatura.*
...
Uma das principais funções de um atributo é **"lembrar" de informações externas** passadas para o objeto.
...
- Podemos ver que ela foi criada com um preço de ingresso de 500 centavos (R$ 5,00).
:::
...
1. Quem lê a classe vê que realmente queremos inicializar com zero.
  
  - Sem isso, poderia gerar dúvida se esquecemos de inicializar ou se queríamos deixar o valor padrão.
:::
...
## Quiz 2.2 {background-color="#f7fad1"}

::: {.nonincremental}
[Marque a alternativa _incorreta_:]{.alert}
...
![](imagens/diagrama_passagem_parametros.png){width=75%}
...
![](imagens/diagrama_passagem_parametros.png){width=50%}

O valor é copiado para o parâmetro `custoIngresso` do construtor da nova máquina (_linha A_).

- A caixa rosa ilustra o espaço adicional que é criado para o objeto somente quando o construtor é chamado.
...
Portanto, no exemplo anterior:

- `custoIngresso` é o parâmetro formal 
- e o valor `500` é o argumento.
...
Repare que o tempo de vida de um parâmetro é diferente do **tempo de vida de um atributo**.
...
No diagrama, a atribuição é representada pela seta indica pela _letra B_.
...
## Quiz 2.3 {background-color="#f7fad1"}

::: {.nonincremental}
[Complete a frase: _______ têm tempo de vida igual ao do objeto:]{.alert}
...
- Obs.: nesse material vou preferir usar sempre `definir`, mas fique à vontade para escolher o que prefere.
:::
:::
...
## Quiz 2.4 {background-color="#f7fad1"}
...
```
4. O último comando reinicializa o saldo como zero, esperando o próximo usuário.
...
O que seria exibido se digitássemos `"preco"` em vez de `preco`?
...
- Seria exibida a palavra _preco_ e não o valor do atributo `preco`.
...
O método deve ter retorno `void`, não ter nenhum parâmetro e exibir uma mensagem como a mostrada abaixo.
