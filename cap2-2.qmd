...
As **aulas práticas são importantes** para que consigam entender os conceitos e saber como aplicá-los em um programa real.

- E, para conseguir acompanhar a aula, é **fundamental ter estudado** o conteúdo da **aula teórica anterior** [antes]{.alert} **da aula prática**.
- Portanto, se tiver faltado da aula teórica, estude o conteúdo em casa antes da aula prática.
:::
...
::: {.column width="80%"}
::: {.callout-note title="Presença" appearance="simple" icon=false}
...
E não se esqueça de enviar suas atividades ao terminar aula mesmo que não tiver terminado.
...
Para fazer os exercícios, baixe o projeto [maquina-ingressos-ingenua](https://github.com/ufla-ipoo/maquina-ingressos-ingenua) original da aula anterior.

- Você pode estar se perguntando se não seria melhor usar o projeto com as alterações feitas 
  durante a aula teórica.
- É que as alterações que faremos aqui nessa aula não dependem das alterações que tínhamos feito lá.
:::
...
Responda escrevendo um comentário dentro do corpo (bloco de código) do construtor.
:::
...
Responda escrevendo um comentário dentro do corpo (bloco de código) do segundo construtor.
:::
...
Acrescente um comentário no cabeçalho do método dizendo se esse é um **método de acesso** ou um **método modificador** e justificando sua resposta.
...
- Lembre-se de testar casos de ingressos com valores que tenham centavos "quebrados", como R$ 4,50.
:::
...
# Questionário de Revisão  {background-color="#40666e"}

##

::: {.callout-important title="Questionário no Campus Virtual"}
Acesse e responda, no Campus Virtual, o **Questionário de Revisão** da Aula 2.2.
:::

# Apenas para os curiosos - Parte 2  {background-color="#40666e"}

## {.smaller}

::: {.nonincremental}
Os exercícios a seguir são uma continuação dos exercícios opcionais do **projeto da [nave](https://github.com/ufla-ipoo/nave)**, tratados na aula anterior.

- Esses exercícios também são opcionais (não precisam ser entregues).
- Mas são muito interessantes para reforçar os conceitos e, ao mesmo tempo, para conhecer mais sobre a linguagem Java.
  - E ainda vai se divertir jogando! 🚀
- Obs.: se não tinha feito os exercícios na aula passada, comece por eles antes de continuar aqui.

Fazendo esses exercícios você terá um **jogo mais interessante**:
...
- Deu certo, conforme esperado? Senão, corrija o que ficou faltando.
:::
...
A gente tinha facilitado as coisas colocando o nível de proteção.
...
- Altere o método `contarMorteInimigo` de forma que, a cada 500 pontos, o nível atual seja incrementado em 1.
  ...
  - Dica: basta acrescentar uma condição que verifique o resto da divisão por zero como mostrado abaixo.
...
- Para isso, procure o método `desenharPlacar` na classe `Cenario` e encontre a linha abaixo:
...
- Vamos alterar esse método para que ele receba o nível atual do jogo e defina a velocidade do inimigo de acordo com o nível.
  - Para isso: acrescente um parâmetro no método para receber o nível atual.
