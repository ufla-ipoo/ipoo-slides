...
Como podemos tratar isso? (*Pense na solução antes de continuar.*)
...
::: {.callout-note title="(OPCIONAL) Exercício 2.1 - Nível de proteção imprevisível" icon=false}
...
- Altere então a inicialização do atributo `nivelDeProtecao`, retirando o código atual que inicializa com 100 e chamando o método `gerarNivelDeProtecao`.
...
- Para isso, procure o método `desenharPlacar` em `Cenario` e encontre a linha abaixo:
  ...
  ```
- Veja que o primeiro argumento passado para o método `drawString` é a String que aparece na tela com o valor do placar. Concatene à String passada: a palavra *nivel* e o valor do nível atual. 
