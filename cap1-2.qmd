---
title: Cap. 1 Objetos e Classes - Aula Prática
...
- Agora digite os comandos do slide anterior para criar uma pessoa, exibi-la e movê-la para direita.
...
- Crie uma instância da classe `Figura` e chame seu método `desenhar`. Em seguida, experimente os métodos `definirPretoEBranco` e `definirColorido`.
...
- É isso que a classe `Figura` faz.
  - A classe `Figura` é escrita de forma que, quando uma instância é criada, são criados dos objetos quadrado (para a parece e para a janela), um triângulo e um círculo.
...
- Um programa em java tem centenas ou milhares de objetos. O usuário inicia o programa (que geralmente cria o primeiro objeto) e, daí em diante, todos os objetos são criados a partir de outros.
...
- Avalie onde seria possível iniciar o atributo `nome` com o seu nome no código da classe `Nave`. 
...
::: {.callout-note title="Exercício 2.6 - Melhorando a usabilidade" icon=false}
