...
- E, para conseguirmos acompanhar a aula, é fundamental ter estudado o conteúdo da aula teórica anterior **antes da aula prática**.
...
No passo anterior, vimos que o código Java necessário para criar uma pessoa, exibi-la e fazê-la mover para a direita seria o seguinte:
...
- Chamamos métodos usando o nome da variável, seguido de `.` (ponto), seguido do nome do método, seguido da lista de parâmetros (que pode ser vazia) entre parênteses.
...
::: {.callout-note title="(Opcional) Exercício 1.2" icon=false}
...
Cada classe tem um [código-fonte]{.alert} associada a ela; é o texto que define os detalhes da classe.
...
# Apenas para os curiosos  {background-color="#40666e"}
...
- Veja que existem seis classes no projeto: `Nave`, `Inimigo`, `Placar`, `Cenario`, `Tiro` e `TesteJogo`. Vamos começar conhecendo os objetos envolvidos.
...
Faça o mesmo com as seguintes classes: `Inimigo` e `Placar`.
...
- Passe os objetos criados anteriormente e avalie os métodos e atributos da objeto cenário.
:::
...
- Por fim, altere o método `getTextoExibicao` para que ele retorne o valor do `nome`.
...
- Faça com que o jogo exiba, na frente do nome da nave, o nível de proteção da nave.
- Para isso, veja dica no próximo slide.
...
::: {.callout-important title="Entrega no Campus Virtual"}
Acesse, no Campus Virtual, a atividade correspondente à terceira parte dos exercícios e envie o código do projeto `nave` da forma que ficou após as suas alterações (compacte a pasta em um arquivo `.zip` para enviar);
:::