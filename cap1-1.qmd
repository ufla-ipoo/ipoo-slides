...
Além disso, estes slides se baseiam no [Cap. 1 do livro]{.alert}.
...
::: {.column width="30%"}
![](imagens/livro_bluej.png){fig-align="center" fig-alt="Capa do livro do Barnes e Kolling." style="max-height: 30vh; width: auto;"}
...
- _"meu carro velho está parado no estacionamento"_.
...
- Se estiver usando Windows, entre na pasta criada e clique duas vezes no arquivo: **`package.bluej`**.
  - Se estiver usando Linux, abra o **BlueJ**, acesse o menu _Projeto_ --> _Abrir projeto..._ e selecione a pasta criada.
...
- Portanto, `fazerGol` não seria um método válido.
...
- Nesse caso, é a distância que o círculo deve se mover.
...
- Ela exibe o *Inspetor de objetos* que mostra os **valores dos atributos** guardados dentro de um objeto.
...
A classe também define os tipos de cada atributo: 

- os primeiros são `int`, a cor é do tipo `String` e o último é `boolean` (verdadeiro ou falso).
...
::: {.callout-note title="Exercício" icon=false}
Escolha uma das imagens abaixo e tente criar uma imagem parecida usando as formas do projeto `figuras`. Mas seja criativo e utilize cores, posições e tamanhos diferentes. Ao fazer isso, anote o que você teve que fazer para conseguir. Seria possível fazer a mesma coisa de formas diferentes?
:::
...
Vamos agora criar um objeto da classe **Disciplina** no projeto `disciplina`, chamado `ipoo`.
...
Fazendo o exercício podemos perceber que [objetos]{.alert} podem ser [passados como parâmetros]{.alert} para métodos de outros objetos.
...
::: {.callout-note title="Exercício" icon=false .nonincremental}
