...
- Vamos agora começar as fazer modificações para melhorar o design e a implementação do sistema.
...
- Assim, podemos implementarmos a classe `Animal` em uma cópia do projeto e comparar as simulações
...
::: {.callout-note title="Exercício" icon=false}
Discuta sobre como o conceito de herança melhorarou o projeto até agora.
:::
...
- E renomar os métodos de ação específicos (`correr` em `Coelho` e `cacar` em `Raposa`) para `agir`.
...
- Como um [método abstrata]{.alert}.

. . .

::: {.callout-tip title="Conceito" icon=false}
Uma **método abstrato** possui apenas assinatura e não tem um corpo. 
Ele é marcada com a palavra-chave `abstract`.
:::
...
::: {.callout-note title="Exercício" icon=false}
Altere o projeto tratamento o método `agir` conforme discutido anteriormente.
