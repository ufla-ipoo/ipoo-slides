...
- `Campo` representa uma área de duas dimensões onde os animais vivem. 
...
::: {.callout-note title="Exercício" icon=false}
Acione o botão `Reiniciar Simulação` para criar um novo estado inicial para a simulação e
...
Antes de continuarmos, há um detalhe importante sobre a implementação da classe `VisaoDeGrafico`.

- Não entraremos nos detalhes sobre implementação de interface gráfica em Java.
- Mas vale a pena destacar que a classe `VisaoDeGrafico` possui uma classe interna privada chamada
  `PainelDeGrafico`.
  - Ela é privada, porque é um detalhe de implementação da classe `VisaoDeGrafico` e não deve ser acessada por outras classes.
...
- Para verificar isso, defina `IDADE_MAXIMA` com valor `1` na superclasse `Animal` e execute a simulação.
