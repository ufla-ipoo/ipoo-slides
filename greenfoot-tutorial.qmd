...
Baseado no que fizemos para a mensagem de vitória, 

- exiba a mensagem `"Game Over!"` quando todos os caranguejos forem comidos pelas lagostas.
...
A pontuação ainda não aparece, 

- porque precisamos agora chamar o  método `contarPontos` toda vez que o caranguejo comer uma larva.
...
A ideia agora é criar aranhas que também comam larvas, 

- e assim dificultem a vida do jogador na obteção de uma boa pontuação.

. . .

Num primeiro passo, vamos fazer uma versão simplificada da aranha. 

- Ela sempre andará para frente e comerá uma larva se der sorte de ter uma no caminho dela.
...
- e faça com que ela coma uma larva se tocar nela 
  - (com o mesmo tipo de código que usamos para os caranguejos).
...
- O problema é que estamos escolhendo uma nova larva-alvo a cada iteração do Greenfoot.
...
O que precisamos é manter o mesmo alvo e só mudá-lo quando a larva for devorada.

- seja pela aranha, seja pelos caranguejos.

. . .

Podemos fazer isso com a implementação no método `act` mostrada a seguir
