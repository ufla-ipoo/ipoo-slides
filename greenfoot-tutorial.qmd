...
# Parte 6: Exercícios {background-color="#40666e"}

Vamos agora incrementar o nosso jogo, cada um no seu próprio passo.


## 

:::: {.callout-note title="Exercício 1" icon=false .nonincremental}
Vamos agora aprender a obter informações do usuário e exibir informações na tela do jogo.
Pediremos ao jogador para informar seu nome e o mostraremos na tela.

Para isso você deve:

- Criar um atributo `nomeJogador` na classe `MundoDoCaranguejo`.
- Criar um método `act` na classe `MundoDoCaranguejo` (público, com retorno `void`, sem parâmetros).
  - O método é chamado a cada atualização do Greenfoot assim como é feito para os atores.
- No método você deve verificar se o atributo `nomeJogador` é `null`.
  - Se for, você deve pedir ao usuário seu nome, usando o método `Greenfoot.ask` e guardar o retorno
    do método no atributo `nomeJogador`.
  - Caso contrário, você deve exibir o nome do jogador na tela usando o método `showText`,
    passando como parâmetro o nome do jogador e os valores 200 (posição X) e 10 (posição Y).
::::

##

:::: {.callout-note title="Exercício 2" icon=false .nonincremental}
Já que conseguimos mostrar coisas na tela, seria interessante mostrar a pontuação
do jogador, que poderia ser contada a cada larva que é devorada pelo caranguejo.

Faça as seguintes alterações na classe `MundoDoCaranguejo`:

- Crie um atributo inteiro chamado `pontos`.
- Altere a chamada do método `showText` para incluir o valor do atributo criado.
- Crie um método `contarPonto`, que aumente o número de pontos em 10.

Em seguida, altere o método `act` da classe `Caranguejo` para chamar o
método `contarPonto` que acabamos de criar na classe `MundoDoCaranguejo`
sempre que o caranguejo comer uma larva.

- Para isso, você precisará obter uma referência para o `MundoDoCaranguejo`
...
:::: {.callout-note title="Exercício 3" icon=false .nonincremental}
...
Nesse passo vamos fazer uma versão simplificada da aranha. 
...
No método `act` faça a aranha movimentar (com `move(2)`) e faça
com que ela coma uma larva se tocar nela (com o mesmo código que usamos
para os caranguejos).
...
:::: {.callout-note title="Exercício 4" icon=false .nonincremental}
