...
::: {.nonincremental}
- Tipos de classe, operadores lógicos (`&&` e `||`), concatenação de strings, construçãfo de objetos (operador `new`), chamadas de métodos, `this`.
:::
...
d. Visão estática se referente ao tempo de execução do programa.
:::
...
::: {.callout-note title="Exercício de Implementação 2" icon=false}
...
- Responder, no Campus Virtual, aos quizes presentes nesses slides e enviar os projetos dos exercícios de implementação 1 e 2.
:::
:::