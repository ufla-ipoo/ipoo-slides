...
::: {.nonincremental}
- Classes como tipos, operadores lógicos (`&&` e `||`), concatenação de strings, construção de objetos (operador `new`), chamadas de métodos, `this`.
:::
...
## Quiz 3.1 {background-color="#f7fad1"}
...
- Os atributos `horas` e `minutos` são do tipo `VisorDeNumero`.
- Que é a classe que criamos para representar os objetos que mostram os números.
...
## Quiz 3.2 {background-color="#f7fad1"}
...
::: {.callout-note title="Exercício" icon=false}
No BlueJ acesse a opção: **Exibir** &rarr; **Exibir terminal**, e no terminal acesse **Opcoes** &rarr; **Anotar chamadas de métodos**.
...
Agora que já entendemos como criar uma classe que define um visor de números de dois dígitos, vamos estudar o projeto [visor-relogio](https://github.com/ufla-ipoo/visor-relogio).
...
- Inclusive `stringVisor`, já que uma string é um objeto em Java.
  - [Obs.: por simplicidade, o diagrama não mostra a representação interna do objeto String.]{style="font-size: 70%;"}
...
## Quiz 3.3 {background-color="#f7fad1"}
...
## Quiz 3.4 {background-color="#f7fad1"}
...
::: {.callout-note title="Exercício de Implementação 1" icon=false}
...
Portanto, os minutos devem ir de 0 a 59, mas as horas devem ir de 1 a 12.
:::

##

::: {.callout-note title="(Opcional) Exercício de Implementação 2" icon=false}
